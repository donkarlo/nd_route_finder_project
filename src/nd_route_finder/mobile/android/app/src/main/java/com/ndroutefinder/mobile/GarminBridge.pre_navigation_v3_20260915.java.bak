...
public final class GarminBridge {
    ...
    public void sendPointAndOpenNativeNavigation(double lat, double lon, String name) {
        // Queue the target for ND Route Target first. If no activity is active yet,
        // Garmin Connect keeps the message for the data field until the selected
        // activity starts and the data field becomes active. The data field then
        // calls routeTo() and makes the native activity calculate the route.
        sendPoint(DATAFIELD_APP_ID, lat, lon, name, false);

        // Send the same target to the receiver and open it so the watch can show
        // Garmin's native activity chooser (Walking, Run, Bike, ...).
